...
test "setup features" {
    ...
    // Test: all features enabled
    {
        ...
        try setupFeatures(&env, .{ .cursor = true, .sudo = true, .title = true, .@"ssh-env" = true, .@"ssh-terminfo" = true, .@"ssh-persist" = true, .path = true }, true);
        try testing.expectEqualStrings("cursor:blink,path,ssh-env,ssh-persist,ssh-terminfo,sudo,title", env.get("GHOSTTY_SHELL_FEATURES").?);
    }
